...
# TODO: CRIO_TASK_MODULE_SERVER_CONFIGURATION
    ...
    linuxType=`cat /etc/os-release | grep 'NAME' | head -1 |  grep -Eo '[[:alpha:]]+|[0-9]+' | tail -1`
...
# TODO: CRIO_TASK_MODULE_SERVER_CONFIGURATION
    ...
    ubuntuVersion=`cat /etc/os-release | grep 'VERSION_ID' | head -1 |  grep -Eo '[[:alpha:]]+|[0-9]+' | head -3 | tail -1`  
